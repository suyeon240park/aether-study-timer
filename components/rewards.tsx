...
export default function Rewards({ studyData }: RewardsProps) {
  ...
  useEffect(() => {
    ...
    // Calculate minutes accumulated towards the next aether (1 aether per 30 minutes)
    const accumulated = totalMinutes % 30

    // Calculate remaining minutes to next aether
    const remaining = 30 - accumulated

    setRemainingMinutes(accumulated)
  }, [studyData])
