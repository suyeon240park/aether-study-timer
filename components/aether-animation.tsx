...
export default function AetherAnimation({
  ...
  useEffect(() => {
    // Create aethers with random offsets for a more natural animation
    const newAethers = Array.from({ length: aetherCount }, (_, i) => {
      const randomOffsetX = Math.random() * 60 - 30 // -30 to +30px
      const randomOffsetY = Math.random() * 60 - 30 // -30 to +30px
      const randomDelay = Math.random() * 0.5 // 0 to 0.5s
    ...
    <>
      {aethers.map((aether) => (
        <Diamond
          ...
          data-end-x={endPosition.x}
          data-end-y={endPosition.y}
        />
      ))}
      ...
      <style jsx global>{`
        ...
        @keyframes aetherFlyToCorner {
          ...
          100% {
            transform: translate(
              ${(props: any) => props.dataset.endX - props.dataset.startX}px,
              ${(props: any) => props.dataset.endY - props.dataset.startY}px
